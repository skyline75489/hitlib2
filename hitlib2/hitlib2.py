...
class Book(dict):
    ...
    def shelf(self):
        for i in self._get_result():
            print(unicode(
                '{0[num]}\t\t{0[pos]}\t\t{0[type]}\t\t{0[status]}\t{0[return]}').format(i))
...
def get_parser():
    parser = argparse.ArgumentParser(
        description='Tiny Python library servering as a set of APIs for HIT Online Library')
    parser.add_argument('query', metavar='QUERY', type=str, nargs='*',
                        help='query the book info')
    parser.add_argument(
        '-t', '--type', help='book type of query: sm (shumu), qk (qikan), lw (lunwen)', default='sm', type=str)
    parser.add_argument(
        '-a', '--all', help='display the full info of the result')
    parser.add_argument(
        '-p', '--pos', help='select book info in specified position (default: 1)')
